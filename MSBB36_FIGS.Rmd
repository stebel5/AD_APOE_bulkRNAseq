...
e4neg<- read.csv("msbb36_E4neg_ADvC.csv",header= TRUE, sep = ",", stringsAsFactors=FALSE)
e4pos<- read.csv("msbb36_E4pos_ADvC.csv",header= TRUE, sep = ",", stringsAsFactors=FALSE)
e4neg$X<- NULL
e4pos$X<- NULL
...
range(full.markers$log2FoldChange.x) # -1.503384  3.413270
range(full.markers$log2FoldChange.y) # -1.121890  1.535588
...
#pdf("2021_04_25_msbb36_lfc.pdf")
ggplot(full.markers, aes(x= log2FoldChange.x, y=log2FoldChange.y, color= sig)) + geom_point(size=0.5) + labs(title="MSBB36", color="Adjusted p-value (BH)") + xlab("APOE4 Negative Log2 FC") + ylab("APOE4 Positive Log2 FC") + theme_bw() + scale_y_continuous(breaks = seq(-2,4, 1)) + scale_x_continuous(breaks = seq(-2,4, 1)) + theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + geom_vline(xintercept = 0) +geom_hline(yintercept = 0) +  scale_color_manual(values=my_col) + theme(legend.position="none", axis.text=element_text(size=20), axis.title=element_text(size=20), plot.title = element_text(size=20)) +  geom_text(data=subset(full.markers,sig2 == "1" & abs(log2FoldChange.x) > 0.4 |sig2 == "1" & abs(log2FoldChange.y) > 0.4), size=7,check_overlap= TRUE, aes(label = external_gene_name))  + geom_vline(xintercept=c(-0.4,0.4), linetype="dotted") + geom_hline(yintercept=c(-0.4,0.4), linetype="dotted")+ expand_limits(x= -1.6) + expand_limits(x= 3.7) 
...
#pdf("msbb36_e4neg_ad_v_c_volcano.pdf")
ggplot(results, aes(log2FoldChange, -log10(pvalue))) +  labs(title="MSBB36 APOE4-Negative [1161 up, 519 down]") + geom_point(aes(col=Significance)) + scale_color_manual(values=c("goldenrod", "dodgerblue", "lightgrey")) +geom_text_repel(data=filter(results, padj<0.05 & abs(log2FoldChange) > 1.25) , aes(label=external_gene_name))
#dev.off()
...
#pdf("msbb36_e4pos_ad_v_c_volcano.pdf")
ggplot(results, aes(log2FoldChange, -log10(pvalue))) +  labs(title="MSBB36 APOE4-Positive [12 up, 7 down]") + geom_point(aes(col=Significance)) + scale_color_manual(values=c("goldenrod", "dodgerblue", "lightgrey")) +geom_text_repel(data=filter(results, padj<0.05 & abs(log2FoldChange) > 0.6) , aes(label=external_gene_name))
#dev.off()
```
...
length(which(all_markers$count > 1)) #7352
which(all_markers$external_gene_name == "TP73")  #  11 3687
all_markers<- all_markers %>%
   arrange(external_gene_name)
which(all_markers$external_gene_name ==  "TP73")  # 6639 6640
...
colnames(all_markers)
range(all_markers) # -1.503384  3.413270
...
#pdf("2021_04_25_msbb36_fulldeg_heatmaps.pdf")
paletteLength <- 50
...
grid.text("Shared MSBB36 DEGs",x = 0.7, y=0.98, gp=gpar(fontsize=17))
#dev.off()
```
...
setwd("/home/Box/Bulk_RNASeq_Data/MSBB")
write.csv(cluster_summary1, file="MSBB36_E4neg_ADvC_up_GO.csv")
write.csv(cluster_summary2, file="MSBB36_E4neg_ADvC_down_GO.csv")
write.csv(cluster_summary3, file="MSBB36_E4pos_ADvC_up_GO.csv")
write.csv(cluster_summary4, file="MSBB36_E4pos_ADvC_down_GO.csv")
```
...
setwd("/home/Box/Bulk_RNASeq_Data/MSBB")
write.csv(kegg_summ1, file="MSBB36_E4neg_ADvC_up_KEGG.csv")
write.csv(kegg_summ2, file="MSBB36_E4neg_ADvC_down_KEGG.csv")
write.csv(kegg_summ3, file="MSBB36_E4pos_ADvC_up_KEGG.csv")
write.csv(kegg_summ4, file="MSBB36_E4pos_ADvC_down_KEGG.csv")
```
