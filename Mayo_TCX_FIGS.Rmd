---
title: "Mayo_TCX_RNASeq_FIGS"
author: "Stella Belonwu"
date: "06/08/2021"
output: html_document
---
Plots for Mayo TCX DE results\
...
e4neg<- read.csv("mayo_tcx_E4neg_ADvC.csv",header= TRUE, sep = ",", stringsAsFactors=FALSE)
e4pos<- read.csv("mayo_tcx_E4pos_ADvC.csv",header= TRUE, sep = ",", stringsAsFactors=FALSE)
e4neg$X<- NULL
e4pos$X<- NULL
...
full.markers<- dplyr::full_join(e4neg[,c(7,6,3)],e4pos[,c(7,6,3)], by ="external_gene_name") #18512
full.markers<- na.omit(full.markers) #18149
...
range(full.markers$log2FoldChange.x) # -2.104275  3.928666
range(full.markers$log2FoldChange.y) #-1.680970  3.745883
...
#pdf("mayotcx_lfc.pdf")
ggplot(full.markers, aes(x= log2FoldChange.x, y=log2FoldChange.y, color= sig)) + geom_point(size=0.5) + labs(title="Mayo Temporal Cortex", color="Adjusted p-value (BH)") + xlab("APOE4 Negative Log2 FC") + ylab("APOE4 Positive Log2 FC") + theme_bw() + scale_y_continuous(breaks = seq(-2,4, 1)) + scale_x_continuous(breaks = seq(-2,4, 1)) + theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + geom_vline(xintercept = 0) +geom_hline(yintercept = 0) +  scale_color_manual(values=my_col) + theme(legend.position="none", axis.text=element_text(size=20), axis.title=element_text(size=20), plot.title = element_text(size=20)) +  geom_text(data=subset(full.markers,sig2 == "1" & abs(log2FoldChange.x) > 0.4 |sig2 == "1" & abs(log2FoldChange.y) > 0.4), size=7,check_overlap= TRUE, aes(label = external_gene_name))  + geom_vline(xintercept=c(-0.4,0.4), linetype="dotted") + geom_hline(yintercept=c(-0.4,0.4), linetype="dotted")
...
#pdf("mayo_tcx_e4neg_ad_v_c_volcano.pdf")
ggplot(results, aes(log2FoldChange, -log10(pvalue))) +  labs(title="Mayo Temporal Cortex APOE4-Negative [698 up, 527 down]") +
...
#pdf("mayo_tcx_e4pos_ad_v_c_volcano.pdf")
ggplot(results, aes(log2FoldChange, -log10(pvalue))) +  labs(title="Mayo Temporal Cortex APOE4-Positive [251 up, 46 down]") +
geom_point(aes(col=Significance)) + scale_color_manual(values=c("goldenrod", "dodgerblue", "lightgrey")) +geom_text_repel(data=filter(results, padj<0.05 & abs(log2FoldChange) >1.25) , aes(label=external_gene_name))
#dev.off()
```
...
length(which(all_markers$count > 1)) #11606
which(all_markers$external_gene_name == "HES4")  # 3 5806
all_markers<- all_markers %>%
   arrange(external_gene_name)
which(all_markers$external_gene_name ==  "HES4")  # 4437 4438
...
colnames(all_markers)
range(all_markers) #-2.104275  3.745883
...
#pdf("mayo_tcx_fulldeg_heatmaps.pdf")
paletteLength <- 50
...
grid.text("Shared Mayo TCX DEGs",x = 0.7, y=0.98, gp=gpar(fontsize=17))
#dev.off()
```
...
write.csv(cluster_summary1, file="Mayo_TCX_E4neg_ADvC_up_GO.csv")
write.csv(cluster_summary2, file="Mayo_TCX_E4neg_ADvC_down_GO.csv")
write.csv(cluster_summary3, file="Mayo_TCX_E4pos_ADvC_up_GO.csv")
write.csv(cluster_summary4, file="Mayo_TCX_E4pos_ADvC_down_GO.csv")
```
...
setwd("/home/Box/Bulk_RNASeq_Data/Mayo_TCX")
write.csv(kegg_summ1, file="Mayo_TCX_E4neg_ADvC_up_KEGG.csv")
write.csv(kegg_summ2, file="Mayo_TCX_E4neg_ADvC_down_KEGG.csv")
write.csv(kegg_summ3, file="Mayo_TCX_E4pos_ADvC_up_KEGG.csv")
write.csv(kegg_summ4, file="Mayo_TCX_E4pos_ADvC_down_KEGG.csv")
```
