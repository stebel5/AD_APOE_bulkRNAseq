...
---
Plots for Mayo CBE DE results
diag [1:2] apoe[3:4] sex [5:6] age [8:11] batch [12:17]
...
e4neg<- read.csv("mayo_cbe_E4neg_ADvC.csv",header= TRUE, sep = ",", stringsAsFactors=FALSE)
e4pos<- read.csv("mayo_cbe_E4pos_ADvC.csv",header= TRUE, sep = ",", stringsAsFactors=FALSE)
e4neg$X<- NULL
e4pos$X<- NULL
...
full.markers<- dplyr::full_join(e4neg[,c(7,6,3)],e4pos[,c(7,6,3)], by ="external_gene_name") #18368
full.markers<- na.omit(full.markers) #17652
...
range(full.markers$log2FoldChange.x) # -1.642585  2.610110
range(full.markers$log2FoldChange.y) #-1.124115  2.259518
...
#pdf("mayocbe_lfc.pdf")
ggplot(full.markers, aes(x= log2FoldChange.x, y=log2FoldChange.y, color= sig)) + geom_point(size=0.5) + labs(title="Mayo Cerebellum", color="Adjusted p-value (BH)") + xlab("APOE4 Negative Log2 FC") + ylab("APOE4 Positive Log2 FC") + theme_bw() + scale_y_continuous(breaks = seq(-2,4, 1)) + scale_x_continuous(breaks = seq(-3,4, 1)) + theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + geom_vline(xintercept = 0) +geom_hline(yintercept = 0) +  scale_color_manual(values=my_col) + theme(legend.position="none", axis.text=element_text(size=20), axis.title=element_text(size=20), plot.title = element_text(size=20)) +  geom_text(data=subset(full.markers,sig2 == "1" & abs(log2FoldChange.x) > 0.4 |sig2 == "1" & abs(log2FoldChange.y) > 0.4), size=7,check_overlap= TRUE, aes(label = external_gene_name))  + geom_vline(xintercept=c(-0.4,0.4), linetype="dotted") + geom_hline(yintercept=c(-0.4,0.4), linetype="dotted") + expand_limits(x= 3) 

ggplot(full.markers, aes(x= log2FoldChange.x, y=log2FoldChange.y, color= sig)) + geom_point(size=0.5) + labs(title="Mayo Cerebellum", color="Adjusted p-value (BH)") + xlab("APOE4 Negative Log2 FC") + ylab("APOE4 Positive Log2 FC") + theme_bw() + scale_y_continuous(breaks = seq(-2,4, 1)) + scale_x_continuous(breaks = seq(-2,4, 1)) + theme(panel.border = element_blank(), panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black")) + geom_vline(xintercept = 0) +geom_hline(yintercept = 0) +  scale_color_manual(values=my_col) + theme(legend.text = element_text( size = 20), legend.title = element_text( size = 20), axis.text=element_text(size=20), axis.title=element_text(size=20), plot.title = element_text(size=20)) +  geom_text(data=subset(full.markers,sig2 == "1" & abs(log2FoldChange.x) > 0.4 |sig2 == "1" & abs(log2FoldChange.y) > 0.4), size=7,check_overlap= TRUE, aes(label = external_gene_name))  + geom_vline(xintercept=c(-0.4,0.4), linetype="dotted") + geom_hline(yintercept=c(-0.4,0.4), linetype="dotted") + guides(color = guide_legend(override.aes = list(size=5)),text = guide_legend(label = FALSE)) + expand_limits(x= 3) 
#dev.off()
```
...
#pdf("mayo_cbe_e4neg_ad_v_c_volcano.pdf")
ggplot(results, aes(log2FoldChange, -log10(pvalue))) +  labs(title="Mayo Cerebellum APOE4-Negative [498 up, 443 down]") +
geom_point(aes(col=Significance)) + scale_color_manual(values=c("goldenrod", "dodgerblue", "lightgrey")) +geom_text_repel(data=filter(results, padj<0.05 & abs(log2FoldChange) >1.25) , aes(label=external_gene_name))
#dev.off()
...
#pdf("mayo_cbe_e4pos_ad_v_c_volcano.pdf")
ggplot(results, aes(log2FoldChange, -log10(pvalue))) +  labs(title="Mayo Cerebellum APOE4-Positive [33 up, 14 down]") +
geom_point(aes(col=Significance)) + scale_color_manual(values=c("goldenrod", "dodgerblue", "lightgrey")) +geom_text_repel(data=filter(results, padj<0.05 & abs(log2FoldChange) >1.25) , aes(label=external_gene_name))
#dev.off()
```
...
length(which(all_markers$count > 1)) #10898
which(all_markers$external_gene_name == "HES5")  # 20 5469
all_markers<- all_markers %>%
   arrange(external_gene_name)
which(all_markers$external_gene_name ==  "HES5")  # 3999 4000
...
colnames(all_markers)
range(all_markers) #-1.642585  2.610110
...
#pdf("mayo_cbe_fulldeg_heatmaps.pdf")
paletteLength <- 50
...
#pdf("2021_04_25_UpSet_MayoCBE_shared.pdf")
...
grid.text("Shared Mayo CBE DEGs",x = 0.7, y=0.98, gp=gpar(fontsize=17))
#dev.off()
```
...
all_mayo_genes<- as.character((e4neg$ensembl_gene_id))
...
write.csv(cluster_summary1, file="Mayo_CBE_E4neg_ADvC_up_GO.csv")
write.csv(cluster_summary2, file="Mayo_CBE_E4neg_ADvC_down_GO.csv")
write.csv(cluster_summary3, file="Mayo_CBE_E4pos_ADvC_up_GO.csv")
write.csv(cluster_summary4, file="Mayo_CBE_E4pos_ADvC_down_GO.csv")
```
...
all_mayo_genes2<- unlist(mget(all_mayo_genes, org.Hs.egENSEMBL2EG, ifnotfound = NA))

kegg_summ1<-enrichKEGG(e4neg_up_genes2, organism = "hsa", keyType = "kegg",pvalueCutoff = 0.05, pAdjustMethod = "BH", universe=all_mayo_genes2,minGSSize = 10, maxGSSize = 500, qvalueCutoff = 0.2, use_internal_data = FALSE)
kegg_summ1<- as.data.frame(kegg_summ1)

kegg_summ2<-enrichKEGG(e4neg_down_genes2, organism = "hsa", keyType = "kegg",pvalueCutoff = 0.05, pAdjustMethod = "BH", universe=all_mayo_genes2,minGSSize = 10, maxGSSize = 500, qvalueCutoff = 0.2, use_internal_data = FALSE)
kegg_summ2<- as.data.frame(kegg_summ2)

kegg_summ3<-enrichKEGG(e4pos_up_genes2, organism = "hsa", keyType = "kegg",pvalueCutoff = 0.05, pAdjustMethod = "BH", universe=all_mayo_genes2,minGSSize = 10, maxGSSize = 500, qvalueCutoff = 0.2, use_internal_data = FALSE)
kegg_summ3<- as.data.frame(kegg_summ3)

kegg_summ4<-enrichKEGG(e4pos_down_genes2, organism = "hsa", keyType = "kegg",pvalueCutoff = 0.05, pAdjustMethod = "BH", universe=all_mayo_genes2,minGSSize = 10, maxGSSize = 500, qvalueCutoff = 0.2, use_internal_data = FALSE)
kegg_summ4<- as.data.frame(kegg_summ4)

write.csv(kegg_summ1, file="Mayo_CBE_E4neg_ADvC_up_KEGG.csv")
write.csv(kegg_summ2, file="Mayo_CBE_E4neg_ADvC_down_KEGG.csv")
write.csv(kegg_summ3, file="Mayo_CBE_E4pos_ADvC_up_KEGG.csv")
write.csv(kegg_summ4, file="Mayo_CBE_E4pos_ADvC_down_KEGG.csv")
```
