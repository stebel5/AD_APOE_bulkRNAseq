...
Subset data to Brodmann 22 and match meta and data order & EDA
match design to that of mayo's Batch + Diagnosis + Sex + Age + ApoE2 
```{r}
msbb_meta<- msbb_all_meta[msbb_all_meta$BrodmannArea == "BM22",-1] #65
rownames(msbb_meta)<- msbb_meta$sampleIdentifier 

table(msbb_meta$Diagnosis) #52ad, 13c
table(msbb_meta$ApoE)#33(42),34(21),44(2)
table(msbb_meta$Sex)#41f, 24m
table(msbb_meta$Race)#7B,1H,56W
table(msbb_meta$Age2)#60s(3),70s(17),80s(27),90s(18)
table(msbb_meta$ApoE[msbb_meta$Diagnosis == "AD"]) #33(32), 34(18), 44(1)
table(msbb_meta$ApoE[msbb_meta$Diagnosis != "AD"]) #33(10), 34(3), 44(0)
table(msbb_meta$ApoE[msbb_meta$Sex == "F"]) #33(25), 34(14), 44(2)
table(msbb_meta$ApoE[msbb_meta$Sex != "F"]) #33(17), 34(7), 44(0)
...
msbb_dds= msbb_dds[keep,]  #19088 65 

msbb_dds<- DESeq(msbb_dds) 
resultsNames(msbb_dds) #lists the coefficients

msbb_dds<- msbb_dds[which(mcols(msbb_dds)$betaConv),] #12 rows did not converge #19075 65
```
...
#pdf("msbb22.pca.plots1_bigger.pdf") 
barplot(msbb.pca.var.per, main="Scree Plot_ Percent of Variation (MSBB22 PCA)",xlab="Principal Component", ylab="Percent Variation", ylim=c(0,40))#plot percentage of variation

ggplot(msbb_pca_extra) + geom_point(aes(x=PC1, y=PC2, color= Diagnosis)) + xlab(paste("PC1 - ",msbb.pca.var.per[1], "%", sep="")) + ylab(paste("PC2 - ",msbb.pca.var.per[2], "%", sep="")) + ggtitle("BA 22 PCA: Diagnosis")  + scale_colour_manual(values = dittoColors()[1:2])+ theme(axis.text=element_text(size=20,face="bold"),  plot.title=element_text(size=20,face="bold"),  axis.title=element_text(size=20,face="bold"), legend.text = element_text( size = 20), legend.title = element_text( size = 20,face="bold")) 

ggplot(msbb_pca_extra) + geom_point(aes(x=PC1, y=PC2, color= ApoE2)) + xlab(paste("PC1 - ",msbb.pca.var.per[1], "%", sep="")) +   ylab(paste("PC2 - ",msbb.pca.var.per[2], "%", sep="")) + ggtitle("BA 22 PCA: APOE")  + scale_colour_manual(values = dittoColors()[3:4])+ theme(axis.text=element_text(size=20,face="bold"),  plot.title=element_text(size=20,face="bold"),  axis.title=element_text(size=20,face="bold"), legend.text = element_text( size = 20), legend.title = element_text( size = 20,face="bold"))

ggplot(msbb_pca_extra) + geom_point(aes(x=PC1, y=PC2, color= Sex)) + xlab(paste("PC1 - ",msbb.pca.var.per[1], "%", sep="")) +   ylab(paste("PC2 - ",msbb.pca.var.per[2], "%", sep="")) + ggtitle("BA 22 PCA: Sex") + scale_colour_manual(values = dittoColors()[5:6])+ theme(axis.text=element_text(size=20,face="bold"),  plot.title=element_text(size=20,face="bold"),  axis.title=element_text(size=20,face="bold"), legend.text = element_text( size = 20), legend.title = element_text( size = 20,face="bold")) 

ggplot(msbb_pca_extra) + geom_point(aes(x=PC1, y=PC2, color= Age)) + xlab(paste("PC1 - ",msbb.pca.var.per[1], "%", sep="")) +   ylab(paste("PC2 - ",msbb.pca.var.per[2], "%", sep="")) + ggtitle("BA 22 PCA:Age") + scale_colour_manual(values = dittoColors()[8:11])+ theme(axis.text=element_text(size=20,face="bold"),  plot.title=element_text(size=20,face="bold"),  axis.title=element_text(size=20,face="bold"), legend.text = element_text( size = 20), legend.title = element_text( size = 20,face="bold")) 

ggplot(msbb_pca_extra) + geom_point(aes(x=PC1, y=PC2, color= Batch)) + xlab(paste("PC1 - ",msbb.pca.var.per[1], "%", sep="")) +   ylab(paste("PC2 - ",msbb.pca.var.per[2], "%", sep="")) + ggtitle("BA 22 PCA: Batch") + scale_colour_manual(values = dittoColors()[12:20])+ theme(axis.text=element_text(size=20,face="bold"),  plot.title=element_text(size=20,face="bold"),  axis.title=element_text(size=20,face="bold"), legend.text = element_text( size = 20), legend.title = element_text( size = 20,face="bold")) 
#dev.off()
#Slight batch effect!!
```
...
ggplot(msbb.umap2, aes( UMAP1, UMAP2, color = Batch)) + geom_point() + scale_colour_manual(values = dittoColors()[12:20])+ theme(axis.text=element_text(size=20,face="bold"),  plot.title=element_text(size=20,face="bold"),  axis.title=element_text(size=20,face="bold"), legend.text = element_text( size = 20), legend.title = element_text( size = 20,face="bold")) 
#dev.off() 
```
...
annocolors<- list(Diagnosis = c("AD"= dittoColors()[1], "Control" = dittoColors()[2]), 
             ...
             Batch= c("E007C014"= dittoColors()[12], "E2C014"= dittoColors()[13], "H154B394"= dittoColors()[14], "K75rC014" = dittoColors()[15],"S112B394"= dittoColors()[16], "S113B355"= dittoColors()[17], "S113B394"= dittoColors()[18], "S114B394"= dittoColors()[19], "S115B394"= dittoColors()[20]))

colnames(msbb_meta)
#pdf("msbb22.clustheatmap1.pdf") 
...
range(msbb.pca12.zcores$pc1z) #-2.887169  2.259813
...
range(msbb.pca12.zcores$pc2z) # -2.299643  2.477739
...
```{r}
setwd("/home/Box/Bulk_RNASeq_Data/MSBB")
saveRDS(msbb_dds, "msbb22_dds.rds") 
```
...
```{r}
msbb_dds_neg<- msbb_dds[,msbb_dds$ApoE2 =="E4neg"] #19076 42 
...
setwd("/home/Box/Bulk_RNASeq_Data/MSBB")
write.csv(res_dneg_tb, file="msbb22_E4neg_ADvC.csv")
```

ApoE34 & ApoE44/E4pos AD vs Control
```{r}
msbb_dds_pos<- msbb_dds[,msbb_dds$ApoE2 =="E4pos"] #19076 23 
...
setwd("/home/Box/Bulk_RNASeq_Data/MSBB")
write.csv(res_dpos_tb, file="msbb22_E4pos_ADvC.csv")
```
