...
Subset data to Brodmann 36 and match meta and data order & EDA
match design to that of mayo's Batch + Diagnosis + Sex + Age + ApoE2 
```{r}
msbb_meta<- msbb_all_meta[msbb_all_meta$BrodmannArea == "BM36",-1] #55
rownames(msbb_meta)<- msbb_meta$sampleIdentifier 

table(msbb_meta$Diagnosis) #36ad, 19c
table(msbb_meta$ApoE)#33(39),34(14),44(2)
table(msbb_meta$Sex)#33f, 22m
table(msbb_meta$Race)#1A, 7B,3H, 44W
table(msbb_meta$Age2)#60s(3),70s(16),80s(19),90s(17)
table(msbb_meta$ApoE[msbb_meta$Diagnosis == "AD"]) #33(25), 34(9), 44(2)
table(msbb_meta$ApoE[msbb_meta$Diagnosis != "AD"]) #33(14), 34(5), 44(0)
table(msbb_meta$ApoE[msbb_meta$Sex == "F"]) #33(23), 34(9), 44(1)
table(msbb_meta$ApoE[msbb_meta$Sex != "F"]) #33(16), 34(5), 44(1)
...
msbb_dds<- msbb_dds[which(mcols(msbb_dds)$betaConv),] #31 rows did not converge #18318 55 
```
...
#pdf("msbb36.pca.plots1.pdf") 
barplot(msbb.pca.var.per, main="Scree Plot_ Percent of Variation (MSBB36 PCA)",xlab="Principal Component", ylab="Percent Variation", ylim=c(0,40))#plot percentage of variation

ggplot(msbb_pca_extra) + geom_point(aes(x=PC1, y=PC2, color= Diagnosis)) + xlab(paste("PC1 - ",msbb.pca.var.per[1], "%", sep="")) + ylab(paste("PC2 - ",msbb.pca.var.per[2], "%", sep="")) + ggtitle("BA 36 PCA: Diagnosis")  + scale_colour_manual(values = dittoColors()[1:2])+ theme(axis.text=element_text(size=20,face="bold"),  plot.title=element_text(size=20,face="bold"),  axis.title=element_text(size=20,face="bold"), legend.text = element_text( size = 20), legend.title = element_text( size = 20,face="bold")) 

ggplot(msbb_pca_extra) + geom_point(aes(x=PC1, y=PC2, color= ApoE2)) + xlab(paste("PC1 - ",msbb.pca.var.per[1], "%", sep="")) +   ylab(paste("PC2 - ",msbb.pca.var.per[2], "%", sep="")) + ggtitle("BA 36 PCA: APOE")  + scale_colour_manual(values = dittoColors()[3:4])+ theme(axis.text=element_text(size=20,face="bold"),  plot.title=element_text(size=20,face="bold"),  axis.title=element_text(size=20,face="bold"), legend.text = element_text( size = 20), legend.title = element_text( size = 20,face="bold")) 

ggplot(msbb_pca_extra) + geom_point(aes(x=PC1, y=PC2, color= Sex)) + xlab(paste("PC1 - ",msbb.pca.var.per[1], "%", sep="")) +   ylab(paste("PC2 - ",msbb.pca.var.per[2], "%", sep="")) + ggtitle("BA 36 PCA: Sex") + scale_colour_manual(values = dittoColors()[5:6])+ theme(axis.text=element_text(size=20,face="bold"),  plot.title=element_text(size=20,face="bold"),  axis.title=element_text(size=20,face="bold"), legend.text = element_text( size = 20), legend.title = element_text( size = 20,face="bold")) 

ggplot(msbb_pca_extra) + geom_point(aes(x=PC1, y=PC2, color= Age)) + xlab(paste("PC1 - ",msbb.pca.var.per[1], "%", sep="")) +   ylab(paste("PC2 - ",msbb.pca.var.per[2], "%", sep="")) + ggtitle("BA 36 PCA: Age") + scale_colour_manual(values = dittoColors()[8:11])+ theme(axis.text=element_text(size=20,face="bold"),  plot.title=element_text(size=20,face="bold"),  axis.title=element_text(size=20,face="bold"), legend.text = element_text( size = 20), legend.title = element_text( size = 20,face="bold")) 

ggplot(msbb_pca_extra) + geom_point(aes(x=PC1, y=PC2, color= Batch)) + xlab(paste("PC1 - ",msbb.pca.var.per[1], "%", sep="")) +   ylab(paste("PC2 - ",msbb.pca.var.per[2], "%", sep="")) + ggtitle("BA 36 PCA: Batch") + scale_colour_manual(values = dittoColors()[12:20])+ theme(axis.text=element_text(size=20,face="bold"),  plot.title=element_text(size=20,face="bold"),  axis.title=element_text(size=20,face="bold"), legend.text = element_text( size = 20), legend.title = element_text( size = 20,face="bold")) 
#dev.off()
#Slight batch effect!!
```
...
annocolors<- list(Diagnosis = c("AD"= dittoColors()[1], "Control" = dittoColors()[2]), 
             ...
             Batch= c("B18C014"= dittoColors()[12], "E007C014"= dittoColors()[13], "K77C014"= dittoColors()[14], "P19B648" = dittoColors()[15],"S109B355"= dittoColors()[16], "S110B355"= dittoColors()[17], "S111B355"= dittoColors()[18], "S115B355"= dittoColors()[19], "S151B648"= dittoColors()[20]))

colnames(msbb_meta)
#pdf("msbb36.clustheatmap1.pdf") 
...
range(msbb.pca12.zcores$pc1z) #-2.012580  2.111692
...
range(msbb.pca12.zcores$pc2z) # -1.956403  2.554548
...
```{r}
setwd("/home/Box/Bulk_RNASeq_Data/MSBB")
saveRDS(msbb_dds, "msbb36_dds.rds") 
```
...
```{r}
msbb_dds_neg<- msbb_dds[,msbb_dds$ApoE2 =="E4neg"] #18318 39 
...
setwd("/home/Box/Bulk_RNASeq_Data/MSBB")
write.csv(res_dneg_tb, file="msbb36_E4neg_ADvC.csv")
```

ApoE34 & ApoE44/E4pos AD vs Control
```{r}
msbb_dds_pos<- msbb_dds[,msbb_dds$ApoE2 =="E4pos"] #18318 16 
...
setwd("/home/Box/Bulk_RNASeq_Data/MSBB")
write.csv(res_dpos_tb, file="msbb36_E4pos_ADvC.csv")
```
